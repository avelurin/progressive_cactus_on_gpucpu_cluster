...
from snakemake import shell  # Ensure shell functions are available for prefixing and executable settings
...
# ─────────────────────────────────────────────────────────────────────────────
# Ensure all shell commands run inside the correct conda environment
# ─────────────────────────────────────────────────────────────────────────────
# Use bash as the shell for all commands
shell.executable("/bin/bash")
# Initialize conda in non-interactive shells & activate your env
shell.prefix(
...
CACTUS_PATH = ["apptainer", "exec", "--cleanenv", "--bind", "/scratch-scc:/scratch-scc", cactus_image_path]
CACTUS_PATH_TMP = ["apptainer", "exec", "--cleanenv", "--bind", TMPDIR + ":/tmp", "--bind", "/scratch-scc:/scratch-scc", cactus_image_path]

CACTUS_GPU_PATH = ["apptainer", "exec", "--nv", "--cleanenv", "--bind", "/scratch-scc:/scratch-scc", cactus_gpu_image_path]
CACTUS_GPU_PATH_TMP = ["apptainer", "exec", "--nv", "--cleanenv", "--bind", TMPDIR + ":/tmp", "--bind", "/scratch-scc:/scratch-scc", cactus_gpu_image_path]
...
rule preprocess:
    ...
    params:
        path = CACTUS_GPU_PATH_TMP if USE_GPU and config['rule_resources']['preprocess']['gpus'] > 0 else CACTUS_PATH_TMP,
        # select container based on gpu_count from rule_resources
        # gpu_count = lambda wildcards: getRuleResources("preprocess").get("gpus", 0),
        # gpu_count = f"--gpu {config['rule_resources']['preprocess']['gpus']}" if USE_GPU else "",
        # path = CACTUS_GPU_PATH_TMP if getRuleResources("preprocess").get("gpus",0)>0 else CACTUS_PATH_TMP,
    ...
    run:
        ...
        if params.gpu_opt and params.gpu_count > 0:
            cmd += ["--gpu", str(params.gpu_count)];
...
rule blast:
    ...
    params:
        path = CACTUS_GPU_PATH_TMP if USE_GPU and config['rule_resources']['blast']['gpus'] > 0 else CACTUS_PATH_TMP,
        cactus_file = os.path.join(OUTPUT_DIR, CACTUS_FILE),
...
rule align:
    ...
    params:
        ...
        # choose GPU container if any GPUs requested
        # path = CACTUS_GPU_PATH_TMP if USE_GPU else CACTUS_PATH_TMP,
        #config_file = os.path.join(OUTPUT_DIR, CONFIG_FILE),
        # choose GPU container if this rule actually has GPUs
        path = CACTUS_GPU_PATH_TMP if USE_GPU and config['rule_resources']['align']['gpus'] > 0 else CACTUS_PATH_TMP,
        cactus_file = os.path.join(OUTPUT_DIR, CACTUS_FILE),
        node = lambda wildcards: wildcards.internal_node,
        ...
        work_dir = TMPDIR,
        # number of GPUs allocated to this rule (0 for align)
        gpu_opt = USE_GPU,
        gpu_count = config["rule_resources"]["align"]["gpus"],
        # gpu_count = (1 if USE_GPU else 0),
    ...
    run:
        ...
        # explicitly enable GPU mode only if rule_resources says so
        if params.gpu_opt and params.gpu_count > 0:
            cmd += ["--gpu", str(params.gpu_count)];
